...
public class BeanCounterLogicImpl implements BeanCounterLogic {
	...
	private Bean[] beans;
	private int steps;
	private int [] inFlightBeans;
	private int inFlightYPos0;
	private int inFlightYPos1;
	...
	BeanCounterLogicImpl(int slotCount) {
		...
		this.beanCounts = new int [slotCount];
		this.beans = null;
		this.steps = 0;
		this.inFlightBeans = new int[slotCount];
		this.inFlightYPos0 = -1;
		this.inFlightYPos1 = -1;
	}
	...
	public int getRemainingBeanCount() {
		if (beans != null && beans.length >= steps) {
			return beans.length - steps;
		}
		return 0;
	}
	...
	public int getInFlightBeanXPos(int yPos) {
		if (inFlightYPos0 >= 0 && yPos >= inFlightYPos0 && yPos <= inFlightYPos1) {
			return beans[inFlightBeans[yPos]].getXPos();
		}
	...
	public void reset(Bean[] beans) {
		...
		if (this.beans.length > 0) {
			this.steps = 1;
			this.inFlightBeans[0] = 0;
			this.inFlightYPos0 = 0;
			this.inFlightYPos1 = 0;
		}
	}
	...
	public boolean advanceStep() {
		if (inFlightYPos0 <= inFlightYPos1) {
			if (inFlightYPos1 < slotCount - 1) {
				inFlightYPos1 += 1;
			} else {
				beanCounts[beans[inFlightBeans[inFlightYPos1]].getXPos()] += 1;
			}

			for (int i = inFlightYPos1; i > inFlightYPos0; i--) {
				inFlightBeans[i] = inFlightBeans[i - 1];
				beans[inFlightBeans[i]].choose();
			}

			if (steps < beans.length) {
				inFlightBeans[inFlightYPos0] = steps;
			} else {
				inFlightYPos0 += 1;
			}
			steps += 1;
