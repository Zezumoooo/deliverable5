...
import gov.nasa.jpf.vm.ThreadList.Count;
...
public class BeanImpl implements Bean {
	...
	BeanImpl(int slotCount, boolean isLuck, Random rand) {
		...
		SkillAverage=(double)(slotCount-1)*0.5;
		SkillDev=(double)Math.sqrt(slotCount*0.5*(1-0.5));
		SkillLevel=(int)Math.round(rand.nextGaussian()*SkillDev+SkillAverage);
		this.Count=this.SkillLevel;
	}
